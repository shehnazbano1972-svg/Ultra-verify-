...
# Fallback guild ID — used when the OAuth URL has no state=guild_id parameter
# Set this to your Discord server ID so auto-role works even with hardcoded OAuth links
...
<div class="card">
  ...
  <div class="err">{e}</div>
    ...
    if token_resp.status_code != 200:
        ...
        # 429 = Discord rate-limiting — show wait time clearly
...
<div class="card">
  ...
  <p>Discord is temporarily blocking requests from this server (rate limit).<br><br>
  {wait_msg}</p>
  <p style="margin-top:12px;font-size:14px;color:#666">
    ...
    3. Go back to Discord and click the verify button again
        ...
        # "invalid_grant" = code already used (page refresh)
...
<div class="card">
  ...
  <div class="err">{e}\n{traceback.format_exc()[-500:]}</div>
    ...
    # guild_id comes from: session (via /authorize), state param, or GUILD_ID env var fallback
    guild_id = (
        session.pop("guild_id", "")
        or request.args.get("state", "")
        or request.args.get("guild", "")
        or GUILD_ID
    )
