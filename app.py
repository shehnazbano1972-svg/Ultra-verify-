...
def init_db():
    _ensure_dir()
    conn = get_db()
    # Stock of verified users (OAuth tokens)
    conn.execute("""
    ...
    """)
    # Per-guild config — registered when each server runs !verify setup
    conn.execute("""
...
def authorize():
    """
    Verify button in Discord links here: /authorize?guild=GUILD_ID
    We redirect to Discord OAuth with state=guild_id so /callback knows which server.
    """
...
<div class="card">
  ...
  in your Railway dashboard.</p>
</div>"""), 500

    guild_id = request.args.get("guild", "").strip()
    state    = guild_id or secrets.token_urlsafe(16)
    session["oauth_state"]    = state
    session["session_guild"]  = guild_id
...
<div class="card">
  ...
  <h1>Missing Environment Variables</h1>
  {rows}
  <p style="margin-top:12px">Set these in Railway and redeploy, then try again.</p>
</div>"""), 500

    # ── Step 1: Exchange code for tokens (with retry on 429) ──────────────────
  ...
  <div class="err">{last_exc}</div>
    ...
    if token_resp.status_code != 200:
        ...
        if token_resp.status_code == 429:
            ...
            wait_msg = f"Wait <strong>{retry_after} seconds</strong> then try again." if retry_after else \
                       "Wait <strong>60 seconds</strong> then try again."
            return page(f"""
<div class="card">
  <div class="icon">⏳</div>
  <span class="badge yellow">Rate Limited — 429</span>
  <h1>Too Many Requests</h1>
  <p>Discord is temporarily blocking requests from this server.<br><br>{wait_msg}</p>
  <p style="margin-top:12px;font-size:14px;color:#666">
    <strong>What to do:</strong><br>
    1. Close this tab<br>
    2. Wait 60 seconds<br>
    3. Go back to Discord and click the verify button again
  </p>
</div>""")

        if "invalid_grant" in detail or "invalid_grant" in raw_text:
            return page("""
<div class="card">
  <div class="icon">🔄</div>
...
<div class="card">
  ...
  <div class="row"><span>2 — Wrong client secret</span>
    Check <code>DISCORD_CLIENT_SECRET</code> in your Railway dashboard.
  </div>
...
<div class="card">
  ...
  <div class="err">{e}</div>
    ...
    if user_resp.status_code == 401:
        guild_id = session.pop("session_guild", "") or request.args.get("state", "")
        params   = (
            f"client_id={DISCORD_CLIENT_ID}"
            f"&redirect_uri={quote(DISCORD_REDIRECT_URI, safe='')}"
            f"&response_type=code"
...
<div class="card">
  ...
  <div class="err">{e}\n{traceback.format_exc()[-500:]}</div>
    ...
    # ── Step 4: Add to guild + give Verified role ─────────────────────────────
    # guild_id comes from: state param (set by /authorize), or session fallback
    guild_id  = (
        request.args.get("state", "").strip()
        or session.pop("session_guild", "")
    )
    bot_token = os.getenv("BOT_TOKEN", "").strip()
    role_given = False

    if guild_id and bot_token:
        # Look up this guild's Verified role from the database
        try:
            cfg_conn = get_db()
            cfg_row  = cfg_conn.execute(
                "SELECT role_id FROM guild_configs WHERE guild_id = ?", (int(guild_id),)
            ).fetchone()
            cfg_conn.close()
            role_id = str(cfg_row["role_id"]) if cfg_row else None
        except Exception:
            role_id = None
        ...
        try:
            ...
            if role_id and add.status_code in (200, 201, 204):
                requests.put(
    ...
    role_row = (
        ...
        if role_given else
        '<div class="row"><span>Status</span>✅ Saved — you\'ll be added to the server shortly</div>'
    )
...
def config_check():
    ...
    try:
        conn   = get_db()
        total  = conn.execute("SELECT COUNT(*) FROM stock").fetchone()[0]
        guilds = conn.execute("SELECT COUNT(*) FROM guild_configs").fetchone()[0]
        conn.close()
        db_status = f'<span class="ok">✅ Connected ({total} users, {guilds} servers registered)</span>'
    except Exception as e:
...
<div class="card" style="max-width:520px">
  ...
  {row("BOT_TOKEN", os.getenv("BOT_TOKEN",""), "(needed to assign roles)")}
  <div class="row"><span>Database</span>{db_status}</div>
  <div class="row">
    <span>DISCORD_REDIRECT_URI must match Discord → OAuth2 → Redirects exactly</span>
    <code style="word-break:break-all">{DISCORD_REDIRECT_URI or "— not set —"}</code>
  </div>
  <div class="row">
    <span>Multi-server mode</span>
    <span class="ok">✅ Active — each server registers its own role via !verify setup</span>
  </div>
  <div class="row">
...
@app.route("/api/guild-config", methods=["POST"])
@require_secret
def api_guild_config():
    """
    Bot calls this during !verify setup to register the guild's Verified role.
    Body: {"guild_id": 123, "role_id": 456}
    """
    ...
    if not guild_id or not role_id:
        return jsonify({"error": "guild_id and role_id are required"}), 400

    try:
        conn = get_db()
        conn.execute("""
            INSERT OR REPLACE INTO guild_configs (guild_id, role_id, registered_at)
            VALUES (?, ?, datetime('now'))
    ...
    return jsonify({"success": True, "guild_id": guild_id, "role_id": role_id})


@app.route("/api/guild-config/<int:guild_id>", methods=["GET"])
@require_secret
def api_guild_config_get(guild_id):
    conn = get_db()
    row  = conn.execute(
        "SELECT guild_id, role_id, registered_at FROM guild_configs WHERE guild_id = ?",
        (guild_id,)
    ).fetchone()
    conn.close()
    if not row:
        return jsonify({"error": "not found"}), 404
    return jsonify({"guild_id": row["guild_id"], "role_id": row["role_id"],
...
if __name__ == "__main__":
    ...
    print(f"[NAXORA] Multi-server : YES — guild configs stored in DB")
